...
echo "=== AuraLite Realistic Crafting Build System v$VERSION ==="
echo ""
...
# VALIDATION
# ============================================================
RECIPE_DIR="$SRC/data/minecraft/recipes"
RECIPE_COUNT=$(ls -1 "$RECIPE_DIR"/*.json 2>/dev/null | wc -l)

echo "🔍 Validating $RECIPE_COUNT recipe files..."

INVALID=0
DUPE_CHECK=$(ls -1 "$RECIPE_DIR"/*.json | xargs -n1 basename | sort | uniq -d)
if [ -n "$DUPE_CHECK" ]; then
    echo "❌ Duplicate recipe filenames detected:"
    echo "$DUPE_CHECK"
    exit 1
fi
...
if [ $INVALID -gt 0 ]; then
    echo "❌ Build aborted: $INVALID invalid JSON file(s)."
    exit 1
fi

echo "✅ All $RECIPE_COUNT recipes are valid JSON. No duplicates found."
echo ""
...
cat > "$DP/pack.mcmeta" <<EOF
{
  "pack": {
    "pack_format": 15,
    "description": "§b$NAME §7v$VERSION §8- §f${RECIPE_COUNT} realistic recipes for MC 1.20.1"
  }
}
EOF
...
cp "$ROOT/CHANGELOG.md" "$DP/CHANGELOG.md" 2>/dev/null || true

( cd "$DP" && zip -qr "$BUILD/$FILEBASE-datapack-$VERSION.zip" . )
echo "📦 Datapack built: $FILEBASE-datapack-$VERSION.zip"
...
cat > "$FB/pack.mcmeta" <<EOF
{
  "pack": {
    "pack_format": 15,
    "description": "$NAME (Fabric) v$VERSION"
  }
}
EOF
...
cat > "$FB/fabric.mod.json" <<EOF
{
  ...
  "version": "$VERSION",
  "name": "$NAME",
  "description": "Replaces ${RECIPE_COUNT} vanilla recipes with more realistic ones.",
  "authors": ["AlexanderNyr"],
  "contact": {
    "homepage": "https://github.com/AlexanderNyr/AuraLite-Realistic-Crafting",
    "sources": "https://github.com/AlexanderNyr/AuraLite-Realistic-Crafting"
  },
...
cp "$ROOT/CHANGELOG.md" "$FB/CHANGELOG.md" 2>/dev/null || true
...
echo "📦 Fabric mod built: $FILEBASE-fabric-$VERSION.jar"
...
cat > "$FG/pack.mcmeta" <<EOF
{
  "pack": {
    "pack_format": 15,
    "description": "$NAME (Forge) v$VERSION",
...
loaderVersion="[47,)"
license="CC-BY-NC-SA-4.0"
issueTrackerURL="https://github.com/AlexanderNyr/AuraLite-Realistic-Crafting/issues"
...
displayURL="https://github.com/AlexanderNyr/AuraLite-Realistic-Crafting"
description='''
Replaces ${RECIPE_COUNT} vanilla recipes with more realistic ones.
...
cp "$ROOT/CHANGELOG.md" "$FG/CHANGELOG.md" 2>/dev/null || true
cp "$ROOT/pack.png"    "$FG/icon.png"    2>/dev/null || true

( cd "$FG" && zip -qr "$BUILD/$FILEBASE-forge-$VERSION.jar" . )
echo "📦 Forge mod built: $FILEBASE-forge-$VERSION.jar"
...
echo "✅ Build complete. Archives are in: $BUILD"
